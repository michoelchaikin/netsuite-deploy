...
async function createSoapClient(config) {
  ...
  let domain = `https://${config.account.replace('_', '-')}.suitetalk.api.netsuite.com`;
...
async function getNetsuiteFolderID(config, folder, ignoreCache = false) {
  ...
  let folderID = ignoreCache
    ? ''
    : parseInt(nconf.get(`${config.account}:folderIDs:${folder}`));
  ...
  nconf.set(`${config.account}:folderIDs:${folder}`, folderID);
